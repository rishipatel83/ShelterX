  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        // Automatically redirect to login on successful signup
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 p-4">
      <div className="max-w-md w-full bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 p-8">
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Operator Registration</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Request access to the shelter system</p>
        </div>
        ...
        <form onSubmit={handleSignup} className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Operator ID / Username</label>
            <input
              type="text"
              required
              value={username}
